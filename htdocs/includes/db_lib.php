...
class API
{
   ...
   public function tb_report($year, $start_date = '1970-01-01', $end_date='1970-01-01'){
	...
	public function getStatus($params){
		...
		if ($params['test_name'])
		{
			...
			$query = "SELECT (SELECT name FROM specimen_activity where state_id  = sl.state_id) AS state
				...
				ORDER BY activity_state_id DESC, date DESC LIMIT 1";
	...
	public function get_lab_status($params)
	{
	...
	public function get_worklist($params)
	{
		...
		$date_filter_condition = " AND IF (status IN ('Test Rejected', 'Sample Rejected', 'Result Rejected', 'Voided'),
			
									TIMESTAMPDIFF(DAY, recent_date, CURDATE()) <= 14,
			
									DATE(recent_date) <= CURDATE()
								) ";
